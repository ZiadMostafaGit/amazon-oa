# Streaming scan: track each stack-prefix's consecutive-run length, confirm at n samples, back-date the start to the run's first sample.
    ...
    for i in range(m):
        ...
        for name in stack:
            ...
            if run == n and prefix not in confirmed:
                events.append(["start", str(start), name])
